...
use cosmic::iced::platform_specific::{
    ...
    shell::commands::layer_surface::{
        Anchor, KeyboardInteractivity, Layer, destroy_layer_surface, get_layer_surface, set_size,
    },
};
...
use cosmic::{
    ...
    iced::{
        self, Alignment, Color, Event, Length, Limits, Padding, Size, Subscription,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut launched_by_toggle = false;
    ...
    if launched_by_toggle {
        match ipc::send_toggle() {
            Ok(()) => {
                eprintln!("sent toggle to existing cosmic-quake-term daemon");
                return Ok(());
            }
            Err(err) => {
                eprintln!("could not contact existing daemon, starting visible: {err}");
            }
        }
    }
    ...
    settings = settings
        .size(Size::new(1920.0, config.terminal_height()))
        .size_limits(
            Limits::NONE
                .min_width(360.0)
                .min_height(280.0)
    ...
    let flags = Flags {
        ...
        startup_options,
        term_config,
        start_hidden: !launched_by_toggle,
    };
...
pub enum Message {
    ...
    SystemThemeChange,
    TerminalHeightDragStarted,
    TerminalHeightDragMoved(cosmic::iced::Point),
    TerminalHeightDragFinished,
    TerminalHeightResizeTick,
...
pub struct App {
    ...
    terminal_height_drag_active: bool,
    terminal_height_pending: Option<u16>,
    terminal_height_applied: u16,
    #[cfg(feature = "password_manager")]
...
impl App {
    ...
    const TERMINAL_HEIGHT_RESIZE_FRAME_MS: u64 = 32;
    const TERMINAL_HEIGHT_RESIZE_STEP: u16 = 4;

    fn apply_visibility(&mut self, hidden: bool) -> Task<Message> {
        if hidden {
            self.hidden = true;
            self.close_quake_surface()
        } else if let Some(id) = self.window_id {
            self.hidden = false;
            self.core.set_main_window_id(Some(id));
            Task::batch([self.ensure_terminal_tab(), self.update_focus()])
        } else {
            self.hidden = false;
            Task::batch([self.open_quake_surface(), self.ensure_terminal_tab()])
        }
    }

    fn open_quake_surface(&mut self) -> Task<Message> {
        let id = window::Id::unique();

        self.window_id = Some(id);
        self.core.set_main_window_id(Some(id));
        self.terminal_height_applied = self.terminal_height_u32() as u16;

        Task::batch([
            get_layer_surface::<cosmic::Action<Message>>(self.layer_surface_settings(id)),
            self.update_focus(),
        ])
    }

    fn close_quake_surface(&mut self) -> Task<Message> {
        if let Some(id) = self.window_id.take().or_else(|| self.core.main_window_id()) {
            self.core.set_main_window_id(None);
            destroy_layer_surface::<cosmic::Action<Message>>(id)
        } else {
            Task::none()
        }
    }

    fn is_quake_surface(&self, id: window::Id) -> bool {
        self.window_id == Some(id) || self.core.main_window_id() == Some(id)
    }
    ...
    fn terminal_height_u32(&self) -> u32 {
        self.terminal_height().round() as u32
    }

    fn layer_surface_settings(&self, id: window::Id) -> SctkLayerSurfaceSettings {
        SctkLayerSurfaceSettings {
            id,
            layer: Layer::Top,
            keyboard_interactivity: KeyboardInteractivity::OnDemand,
            anchor: Anchor::LEFT.union(Anchor::RIGHT).union(Anchor::TOP),
            namespace: <Self as Application>::APP_ID.to_string(),
            size: Some((None, Some(self.terminal_height_u32()))),
            size_limits: Limits::NONE
                .min_width(Self::MIN_WINDOW_WIDTH)
                .min_height(Self::MIN_WINDOW_HEIGHT)
                .max_width(Self::WINDOW_WIDTH)
                .max_height(Self::MAX_WINDOW_HEIGHT),
            ..Default::default()
        }
    }

    fn has_terminal_tab(&self) -> bool {
        self.pane_model
            .panes
            .iter()
            .any(|(_, tab_model)| tab_model.iter().next().is_some())
    }

    fn ensure_terminal_tab(&mut self) -> Task<Message> {
        if self.has_terminal_tab() || self.term_event_tx_opt.is_none() {
            Task::none()
        } else {
            self.create_and_focus_new_terminal(
    ...
    fn start_terminal_height_drag(&mut self) {
        self.terminal_height_drag_active = true;
        self.terminal_height_pending = None;
    }

    fn drag_terminal_height(&mut self, point: cosmic::iced::Point) -> Task<Message> {
        if !self.terminal_height_drag_active {
            return Task::none();
        }

        let height = Self::normalize_terminal_height(point.y);

        if self.config.terminal_height == height && self.terminal_height_pending == Some(height) {
            Task::none()
        } else {
            self.config.terminal_height = height;
            self.terminal_height_pending = Some(height);
            Task::none()
        }
    }

    fn flush_terminal_height_drag(&mut self) -> Task<Message> {
        let Some(height) = self.terminal_height_pending.take() else {
            return Task::none();
        };

        self.apply_terminal_height(height)
    }

    fn finish_terminal_height_drag(&mut self) -> Task<Message> {
        if !self.terminal_height_drag_active {
            return Task::none();
        }

        self.terminal_height_drag_active = false;

        let height = self
            .terminal_height_pending
            .take()
            .unwrap_or(self.config.terminal_height);

        self.config.terminal_height = height;
        self.apply_terminal_height(height)
    }

    fn apply_terminal_height(&mut self, height: u16) -> Task<Message> {
        let height = Self::normalize_terminal_height(f32::from(height));

        if self.terminal_height_applied == height {
            return Task::none();
        }

        self.terminal_height_applied = height;

        if let Some(id) = self.window_id {
            set_size::<cosmic::Action<Message>>(id, None, Some(u32::from(height)))
        } else {
            Task::none()
        }
    }

    fn normalize_terminal_height(height: f32) -> u16 {
        let clamped = height
            .clamp(Self::MIN_WINDOW_HEIGHT, Self::MAX_WINDOW_HEIGHT)
            .round() as u16;
        let step = Self::TERMINAL_HEIGHT_RESIZE_STEP.max(1);

        (((clamped + (step / 2)) / step) * step).clamp(
            Self::MIN_WINDOW_HEIGHT as u16,
            Self::MAX_WINDOW_HEIGHT as u16,
        )
    }
...
impl Application for App {
    ...
    const APP_ID: &'static str = "dev.big_pants.CosmicQuakeTerm";
    ...
    fn init(mut core: Core, flags: Self::Flags) -> (Self, Task<Self::Message>) {
        ...
        let terminal_height_applied = flags.config.terminal_height().round() as u16;
        ...
        let mut app = Self {
            ...
            hidden: flags.start_hidden,
            window_id,
            terminal_height_drag_active: false,
            terminal_height_pending: None,
            terminal_height_applied,
            #[cfg(feature = "password_manager")]
            password_mgr: Default::default(),
        };

        app.set_curr_font_weights_and_stretches();
        let command = if app.hidden {
            Task::batch([app.update_config(), app.update_title(None)])
        } else {
            Task::batch([
                app.update_config(),
                app.update_title(None),
                app.apply_visibility(false),
            ])
        };
    ...
    fn update(&mut self, message: Self::Message) -> Task<Self::Message> {
        ...
        match message {
            ...
            Message::IpcToggle => {
                return self.apply_visibility(!self.hidden);
            }
            ...
            Message::MouseLeftReleased => {
                if self.terminal_height_drag_active {
                    let task = self.finish_terminal_height_drag();
                    config_set!(terminal_height, self.config.terminal_height);
                    return task;
                } else {
                    return self.update(Message::CopyPrimary(None));
                }
            }
            ...
            Message::TerminalHeightDragStarted => {
                self.start_terminal_height_drag();
            }
            Message::TerminalHeightDragMoved(point) => {
                return self.drag_terminal_height(point);
            }
            Message::TerminalHeightDragFinished => {
                if self.terminal_height_drag_active {
                    let task = self.finish_terminal_height_drag();
                    config_set!(terminal_height, self.config.terminal_height);
                    return task;
            ...
            Message::TerminalHeightResizeTick => {
                return self.flush_terminal_height_drag();
            }
                ...
                if let Some(tab_model) = self.pane_model.active_mut() {
                    ...
                    if tab_model.iter().next().is_none() {
                            ...
                            self.pane_model.panes.close(self.pane_model.focused())
                        {
                            ...
                            return self.apply_visibility(true);
                        }
                    }
                }
    ...
    fn on_close_requested(&self, id: window::Id) -> Option<Self::Message> {
        ...
        self.is_quake_surface(id).then_some(Message::WindowClose)
    }
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        ...
        let pane_grid = PaneGrid::new(&self.pane_model.panes, |pane, tab_model, _is_maximized| {
            ...
            let bottom_bar = widget::row::with_children(vec![
                ...
                widget::button::standard("+")
                    .on_press(Message::TabNew)
                    .into(),
                widget::button::standard("Profiles")
                    .on_press(Message::ToggleContextPage(ContextPage::Profiles))
                    .into(),
            ...
            tab_column = tab_column
                .push(widget::layer_container(bottom_bar).layer(cosmic_theme::Layer::Primary));
        ...
        let resize_handle: Element<'_, Self::Message> = widget::mouse_area(
            widget::container(widget::space::vertical())
                .width(Length::Fill)
                .height(Length::Fixed(8.0)),
        )
        .on_press(Message::TerminalHeightDragStarted)
        .interaction(iced::mouse::Interaction::ResizingVertically)
        .into();

        widget::column::with_children(vec![pane_grid.into(), resize_handle])
            .height(Length::Fill)
            .into()
    }
    ...
    fn subscription(&self) -> Subscription<Self::Message> {
        ...
        let mut subscriptions = vec![
            event::listen_with(|event, _status, _window_id| match event {
                ...
                Event::Mouse(MouseEvent::CursorMoved { position }) => {
                    Some(Message::TerminalHeightDragMoved(position))
                }
        ...
        if self.terminal_height_drag_active {
            subscriptions.push(
                time::every(Duration::from_millis(Self::TERMINAL_HEIGHT_RESIZE_FRAME_MS))
                    .map(|_| Message::TerminalHeightResizeTick),
            );
        }
...
fn quake_ipc_subscription() -> Subscription<Message> {
    Subscription::run(|| {
        stream::channel(
            100,
            |mut emitter: iced::futures::channel::mpsc::Sender<Message>| async move {
                ...
                std::thread::spawn(move || {
                    log::info!("binding IPC socket at {:?}", path);

                    let listener = match UnixListener::bind(&path) {
                        Ok(listener) => listener,
                        Err(err) => {
                            log::error!("failed to bind IPC socket: {err}");
                            return;
                        }
                    };
                    ...
                    for stream in listener.incoming() {
                        ...
                        if reader.read_line(&mut line).is_ok() && line.trim() == ipc::TOGGLE_COMMAND
                        {
                            let _ =
                                iced::futures::executor::block_on(emitter.send(Message::IpcToggle));
                        }
                    }
                });
